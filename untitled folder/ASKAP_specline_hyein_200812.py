...
#
# The original script for WALLABY cube has been written by Bi-Qing For.
#
# Modified: Hyein Yoon
# Email: hyein.yoon@sydney.edu.au
# 
# Modified Date: 12 August 2020
#
################################################################################

################################################################################
##### HEADER - WALLABY (original code)
#
# This is a header from the original script by Bi-Qing For.
#
...
def get_FIRST(ra, dec):
    ...
    """
    from astroquery.vizier import Vizier ##### FLASH - installation: conda install -c astropy astroquery

    print ("Retrieving FIRST sources from Vizier. Depending on server connection, this might take a while....")
    
#    Vizier.ROW_LIMIT = 1000 ##### FLASH - it doesn't work!
...
# fitsimage = ('image.restored.i.SB' + sbid + '.cube.contsub.fits') ##### FLASH - moved after "field" definition
...
fitsimage = ('image.restored.i.SB' + sbid + '.cube.' + field + '.beam00.contsub.fits') ##### FLASH - change name
...
flagging_file = sorted(glob.glob('flagSummary/*.ms.flagSummary')) #Flagging statistic for spectral line - ##### FLASH - file name change
...
##### FLASH - font, color
##### FLASH - comments are added below tables
...
html.write("""<table width="100%" border="0">
			...
			<font color="black">Original script for WALLABY: 21-Oct-2019 by Bi-Qing For (ICRAR/UWA)</font></h4></th>
                   	<th style="width:220"><img style="border:none;" src="" width="220" align="center"></th>
...
html.write("""
              ...
              <p align=left>
              <h4 align="left">Notes:
		<br>- This tool uses ASKAPsoft outputs. FITS-datacubes are needed only for getting major and minor beam sizes (from the header). 
		<br>- The values and plots in the tables below are not currently correct - I used dummy files for some cases to run the script successfully.
                
		<br>- 0) Combining the overview statistics of the cube with the spectra from individual sources 
		<br>- 1) Validation Checking with extracted spectral line domain? With top-five strong radio sources in the field (from Selavy catalog)?
		<br>- 2) Radio sources from continuum images - contacted VAST people (Josh and James): astrometric approaches
		<br>- 3) Total bandwidth: 288-MHz (FLASH) vs 12-MHz (WALLABY) - Should we split the long bandwidth into smaller units for validation check?
		<br>- 4) *** Goal of the tool: Do we aim to provide any METRIC or quantify data quality? (i.e. data is good or bad to do science)
		<br>- 5) Any other additional items to be required (e.g. SELAVY/FIRST/NVSS detections in FOV)?
		<br><br></h4>
              </p>""")
...
html.write("""</td>
                    ...
                    <h2 align="middle">Spectra from five brightest components</h2>
                    <table width="100%" border="1">
                    <tr>
                    <th>Component 01a</th>          
                    <th>Component 02a</th>
                    ...
                    <tr align="middle">
                    <td>
                    <a href="../Figures/plot_profile_comp6a_test.png" target="_blank"><img src="{1}" width="{2}" height="{3}" alt="thumbnail"></a>
                    ...
                    """.format("plot_profile_comp6a_test.png",
                               fig_dir+'/' + "plot_profile_comp6a_test.png",
                               sizeX,
                               sizeY,
                               "plot_profile_comp6a_test.png",
                               fig_dir+'/'+ "plot_profile_comp6a_test.png",
                               sizeX,
                               sizeY,
                               "plot_profile_comp6a_test.png",
...
#                                             
#             <p align=left>
#              <br>* If more than one version of ASKAPsoft is used for the whole reduction, the latest one is reported.<br>
#              ** Does not take into account field rotation. <br><br>  
#              Generated at {0} <br>
#              <i> Report bugs to 
